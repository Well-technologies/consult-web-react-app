...
import {
  useGetAdviserScheduleDetails,
  useCreateAdviceBooking,
...
export const CreateAppointment = ({
  ...
  const timeSlotOptions = useMemo(() => {
    return (
      (scheduleData as any)?.data?.doctor_schedule?.timeslots
      ...
      <div className="p-8 bg-white rounded-lg shadow-sm border border-gray-100 max-w-4xl mx-auto my-10">
        <div className="mb-8 flex justify-between items-center border-b pb-4">
          <h2 className="text-2xl font-bold text-gray-800">
            {t("appointment.form.title") || "Appointment Details"}
          </h2>
        ...
        <form onSubmit={handleSubmit(onSubmit)} className="space-y-6">
          ...
          <div className="bg-gray-50 p-6 rounded-xl border border-gray-200">
            <div className="flex items-center gap-3 mb-4">
              ...
              />
              <label
                htmlFor="is_custom_time"
                className="text-base font-semibold text-gray-700 cursor-pointer"
              >
                {t("appointment.form.custom_time") ||
                  "Set a custom time instead of predefined schedule"}
              </label>
            </div>
              ...
              {isCustomTime ? (
                <div className="space-y-2 max-w-md">
                  ...
                  {/* <p className="text-xs text-gray-500 text-center">
                    Please enter time in 24-hour format (HH:mm:ss)
                  </p> */}
                </div>
              ) : (
                <div className="space-y-2">
                  <FormSelect
                    label={
                      t("appointment.form.schedule.label") ||
                  ...
                  {!isLoadingSchedule && timeSlotOptions.length === 0 && (
                    <p className="text-sm text-amber-600 font-medium">
                      No predefined schedules found for this date. Try a custom
                      time.
                    </p>
                  )}
                </div>
          ...
          <div className="pt-6 border-t flex justify-end">
            ...
            >
              {isCreating
                ? "Creating..."
                : t("appointment.form.button") || "Create Appointment"}
            </Button>
          </div>
        </form>
      </div>
