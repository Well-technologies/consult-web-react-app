...
export const OrderCard = ({ order }: OrderCardProps) => {
  ...
  // Collect all prescription image URLs
  const prescriptionImages = useMemo(() => {
    if (isMedOrder) {
      return (
        order.orderDetails.prescriptiondata
          ?.filter((prescription) => prescription?.prescription_url)
          .map((prescription) => prescription.prescription_url) || []
      );
    } else {
      return (
  ...
  return (
    <>
      <ImageViewerModal
        ...
        showThumbnails={prescriptionImages.length > 1}
      />
          ...
          {isMedOrder ? (
            <div className="grid auto-cols-max grid-flow-col gap-1">
              {order.orderDetails.prescriptiondata &&
                ...
                order.orderDetails.prescriptiondata.map(
                  (prescription, index) => (
                    <button
                      key={index}
                      onClick={() => handleImageClick(index)}
                      className="w-10 h-10 rounded border overflow-hidden hover:opacity-80 transition-opacity cursor-pointer"
                    >
                      <img
