...
export const PreviousMedOrders = ({ data }: { data: MedicineOrderDetails[] | undefined  }) => {
    console.log('PreviousMedOrders', data)
    return (
        <div>
            <h2>Previous Med Orders</h2>
    <ul>
        {data?.map((item) => <li key={item.id}>{item.orderItems?.[0].medication_title}</li>)}
    </ul>
    </div>
    )
