...
export const Tabs = <T extends string | number>({
  ...
  return (
    ...
    >
      <div
        className={clsx(
          "px-1 pt-1 flex-shrink-0",
          variant === "default" &&
            "border-0 border-gray-200 rounded-b-lg rounded-t-md dark:border-gray-700"
        )}
      >
        <div
          className={clsx(
      ...
      {tabs.map(
        ({ value, component }) =>
          value === activeTab && (
            <div key={value} className="flex-1 overflow-y-auto min-h-0">
              {component}
            </div>
          )
      )}
